...
impl Isomap {
    ...
    pub fn fit_transform<'py>(
        ...
        let geodesic_distances =
            self.compute_geodesic_distances(&knn_graph, &distances, n_samples)?;
...
impl Isomap {
    fn build_knn_graph(&self, distances: &Array2<f64>, n_samples: usize) -> Vec<Vec<(usize, f64)>> {
        (0..n_samples)
            ...
            .map(|i| {
                ...
                heap.into_iter().map(|(d, j)| (j, d.into_inner())).collect()
            })
            .collect()
    }
    ...
    fn compute_geodesic_distances(
        ...
        let geodesic_rows: Vec<Vec<f64>> = (0..n_samples)
            .into_par_iter()
            .map(|source| self.dijkstra_single_source(source, knn_graph, n_samples))
            .collect();
...
mod tests {
    ...
    fn test_knn_graph_construction() {
        ...
        for neighbors in &knn_graph {
            assert_eq!(
                neighbors.len(),
                2,
                "Each node should have exactly k neighbors"
            );
        }
    ...
    fn test_geodesic_distances_simple_chain() {
        ...
        let geodesic = isomap
            .compute_geodesic_distances(&knn_graph, &distances, 4)
            .unwrap();
    ...
    fn test_floyd_warshall_correctness() {
        ...
        let geodesic = isomap
            .compute_geodesic_distances(&knn_graph, &distances, 3)
            .unwrap();
